...
# 
# 
# df <- read.table("results/scratch/PASanalysis/sensitive_pasByStage.txt",sep="\t",header=TRUE)
# 
# l1 <- c(nrow(df[df$stage=="L1" & df$PAS_type=="AAUAAA",]),
#         nrow(df[df$stage=="L1" & df$PAS_type=="altPAS",]),
#         nrow(df[df$stage=="L1" & df$PAS_type=="noPAS",]))
...
#         #legend.justification = c(0.5, 0),
#         legend.justification = c(0.5, 0))
# pdf(file="figures/figure3/sensitive_figure3F.pdf",height=2.5,width=4.1667,colormodel="rgb")
# print(g1)
# dev.off()
...
df <- read.table("results/scratch/PASanalysis/stringent_pasByStage.txt",sep="\t",header=TRUE)
