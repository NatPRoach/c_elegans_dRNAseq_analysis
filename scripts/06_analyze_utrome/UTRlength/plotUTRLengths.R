...
# ### Sensitive
# 
# df1 <- read.table("results/scratch/UTRlength/sensitive_utr_lengths.txt",sep = "\t",header = TRUE )
# 
# g1 <- ggplot(df1,aes(x=reorder(stage,x_order),y=utr_length))+
#   # geom_hline(yintercept=50,color="gray60")+
#   # geom_hline(yintercept=150,color="gray60")+
...
#   geom_hline(yintercept=150,colour="#990000", linetype="dashed")+
#   geom_hline(yintercept=250,colour="#990000", linetype="dashed")+
#   geom_violin(draw_quantiles = c(0.50),color="black",fill=our_utrs)+
#   ylab("3'UTR Length")+
#   scale_x_discrete(labels=c("L1","L2","L3","L4","yAd", "mAd", "male","all"))+
#   xlab("")+
...
#         legend.title = element_blank())
# 
# pdf("figures/figure3/sensitive_figure3E.pdf",height=2.5,width=4.25,colormodel="rgb")
# g1
# dev.off()
# 
# ks.test(df1$utr_length[df1$stage=="L1"],df1$utr_length[df1$stage=="L2"])
...
df1 <- read.table("results/scratch/UTRlength/stringent_utr_lengths.txt",sep = "\t",header = TRUE )
